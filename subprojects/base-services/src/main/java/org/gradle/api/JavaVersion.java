...
public final class JavaVersion implements Comparable<JavaVersion> {
    ...
    public static final JavaVersion VERSION_1_11_OR_LATER = new JavaVersion(11);
    private static final JavaVersion[] majorVersions = new JavaVersion[]{
    ...
    private static final Map<String, JavaVersion> majorNameToVersions = init();
    ...
    public static JavaVersion valueOf(String value) {
        ...
        JavaVersion result = majorNameToVersions.get(value);
        if (result == null) {
    ...
    public static JavaVersion[] values() {
        return Arrays.copyOf(majorVersions, majorVersions.length);
    }
    ...
    public static JavaVersion toVersion(Object value) {
        ...
        if (isLegacyVersion(versions)) {
            assertTrue(str, versions.size() > 1 && versions.get(1) > 0);
            return new JavaVersion(versions.subList(1, versions.size()));
        } else {
            assertTrue(str, !versions.isEmpty() && versions.get(0) > 0);
    ...
    public JavaVersion major() {
        if (getMajorVersionNumber() > 10) {
            return JavaVersion.VERSION_1_11_OR_LATER;
        }
        return majorVersions[getMajorVersionNumber() - 1];
    }
